/**
* (c) 2008 Development Lab,
* Stored Procedure Generator 0.2, Steve L. Nyemba
* Generated for: <OWNER>
*
* Features:
...
*	SELECT <SCHEMA>.generateStoredProcedures(<schema>,<table>) ; and the utility will create the appropriate functions in the <schema> you specified.
*
* Things to know:
...
CREATE OR REPLACE FUNCTION <SCHEMA>.getFieldNames(p_schemaName varchar, p_tableName varchar) returns REFCURSOR AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION <SCHEMA>.generateUpdateStoredProcedure(p_schemaName VARCHAR, p_tableName VARCHAR) RETURNS VOID AS $$
DECLARE
	oRecord		RECORD;
	oTableDef	REFCURSOR;
...
BEGIN
    oPrimaryKeyName := <SCHEMA>.getPrimaryKeyName(p_schemaName,p_tableName) ;
	...
	OPEN oTableDef FOR SELECT  <SCHEMA>.getFieldNames(p_schemaName,p_tableName);
...
CREATE OR REPLACE FUNCTION <SCHEMA>.generateInsertStoredProcedure(p_schemaName varchar,p_tableName varchar) returns void as  $$
DECLARE
	oRecord 			RECORD;
	oTableDef			REFCURSOR;
...
END;
$$ LANGUAGE PLPGSQL ;
CREATE OR REPLACE FUNCTION <SCHEMA>.generateGetStoredProcedure(p_schemaName VARCHAR,p_tableName VARCHAR) RETURNS VOID AS $$
DECLARE
	oFunctionBody 	VARCHAR;
	oSQLQuery	VARCHAR;
BEGIN
...
CREATE OR REPLACE FUNCTION <SCHEMA>.generateGetByAgnStoredProcedure(p_schemaName VARCHAR,p_tableName VARCHAR) RETURNS VOID AS $$
DECLARE
	oFunctionBody 	    VARCHAR ;
	oSQLQuery	        VARCHAR;
	oPrimaryKeyName     VARCHAR;
...
CREATE OR REPLACE FUNCTION <SCHEMA>.generateStoredProcedures(p_schemaName varchar,p_tableName varchar) returns void as $$
DECLARE 
	oFieldNames REFCURSOR ;
...
--		OPEN oFieldNames FOR SELECT <SCHEMA>.getFieldNames(p_schemaName,p_tableName) ;
--		FETCH oFieldNames INTO oFieldNames ;
		-- Creating an insert stored procedure
		PERFORM <SCHEMA>.generateInsertStoredProcedure(p_schemaName,p_tableName) ;
		PERFORM <SCHEMA>.generateGetByAgnStoredProcedure(p_schemaName,p_tableName);
		PERFORM <SCHEMA>.generateUpdateStoredProcedure(p_schemaName,p_tableName);
		PERFORM <SCHEMA>.generateGetStoredProcedure(p_schemaName,p_tableName);
--		CLOSE oFieldNames ;
	END IF ;
END;
...
CREATE OR REPLACE FUNCTION <SCHEMA>.getPrimaryKeyName(p_schemaName varchar, p_tableName varchar) returns varchar as $$
DECLARE
